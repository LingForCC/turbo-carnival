      ...
      <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        ...
        <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          ...
          <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            <div>
              <h2 class="text-xl font-semibold text-gray-800 m-0">LLM Providers</h2>
  ...
  private renderProviderList(): string {
    if (this.providers.length === 0) {
      return `
        <div class="text-center py-8">
          <p class="text-sm text-gray-400 mb-4">No providers configured yet</p>
          <button id="add-provider-btn" class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg cursor-pointer border-0">
    ...
    return `
      <div class="flex justify-between items-center mb-4">
        ...
        <button id="add-provider-btn" class="flex items-center gap-2 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded text-sm font-medium cursor-pointer border-0">
  ...
  private renderProviders(): string {
    return this.providers.map(provider => `
      <div class="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-2">
            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium
              ${provider.type === 'openai' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'}">
              ${provider.type.toUpperCase()}
            </span>
            <span class="text-sm font-medium text-gray-800">${this.escapeHtml(provider.name)}</span>
          </div>
        ...
        <div class="flex items-center gap-2">
          <button class="edit-provider-btn p-1.5 hover:bg-gray-100 rounded cursor-pointer border-0 bg-transparent"
          ...
          <button class="delete-provider-btn p-1.5 hover:bg-red-100 rounded cursor-pointer border-0 bg-transparent"
  ...
  private renderProviderForm(): string {
    ...
    return `
      ...
      <form id="provider-form" class="space-y-4">
        <!-- Provider Type -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="provider-type">
        ...
        <!-- Provider ID -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="provider-id">
          ...
          ${isEdit ?
            '<p class="text-xs text-gray-400 mt-1">Provider ID cannot be changed</p>' :
        ...
        <!-- Display Name -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="provider-name">
        ...
        <!-- API Key -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="provider-apikey">
        ...
        <!-- Base URL -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1" for="provider-baseurl">

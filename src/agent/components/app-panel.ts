...
import { AppCodeMessage } from '../../conversation/components/app-code-message';
...
import { getAgentManagementAPI } from '../api';
import type { AgentManagementAPI } from '../types';
...
export class AppPanel extends HTMLElement {
        ...
        ${!this.showingPreview ? `
          ...
          <div class="h-16 flex items-center justify-between px-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
            ...
            <div class="flex items-center gap-2">
              ${this.currentAgent ? `
                <button id="archive-chat-btn" class="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded cursor-pointer border-0 bg-transparent" title="Archive conversation">
  ...
  private attachEventListeners(): void {
    ...
    // Archive chat button
    const archiveBtn = this.querySelector('#archive-chat-btn');
    if (archiveBtn) {
      const newBtn = archiveBtn.cloneNode(true);
      archiveBtn.replaceWith(newBtn);
      (newBtn as HTMLElement).addEventListener('click', () => this.archiveChat());
    }
  ...
  private async archiveChat(): Promise<void> {
    if (!this.currentAgent || !this.currentProject) return;

    const confirmed = confirm(
      'Archive the conversation history? This will create a backup file and clear the current conversation.'
    );

    if (confirmed) {
      try {
        const result = await this.agentAPI.archiveAgentHistory(
        ...
        alert(`Conversation archived to: ${result.archivedFileName}`);
      } catch (error: any) {
        console.error('Failed to archive conversation:', error);
        alert(`Failed to archive conversation: ${error.message || 'Unknown error'}`);
      }
    }
  }
